    ...
    for num_rows in [12, 10]:
        for num_cols in [15, 9]:
            env = Four_Rooms_Environment(
                grid_width=num_cols, grid_height=num_rows)
            observed_states = set()
...
def test_check_user_location_and_goal_location_match_state_and_next_state():
    ...
    for _ in range(50):
        ...
        for _ in range(50):
            move = randint(0, 3)
            env.step(move)
            assert env.state == [env.location_to_state(
                env.current_user_location), env.location_to_state(env.current_goal_location)]
            assert env.next_state == [env.location_to_state(
                env.current_user_location), env.location_to_state(env.current_goal_location)]
...
def test_location_to_state_and_state_to_location_match():
    ...
    for row in range(env.grid_height):
        for col in range(env.grid_width):
            assert env.location_to_state((row, col)) == env.location_to_state(
                env.state_to_location(env.location_to_state((row, col))))
...
def test_randomness_of_moves():
    ...
    for move in env.actions:
        ...
        for move_test in env.actions:
            if move != move_test:  # We do this because stochastic probability 1.0 means the move will never be picked
                assert abs((num_iterations / (len(env.actions)-1)) -
                           count[move_test]) < num_iterations / 20.0,  "{}".format(count)
    ...
    for move in env.actions:
        ...
        count = Counter(moves)
        for move_test in env.actions:
            assert abs((num_iterations / len(env.actions)) -
                       count[move_test]) < num_iterations / 20.0, "{}".format(count)
