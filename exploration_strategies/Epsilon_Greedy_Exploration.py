...
from exploration_strategies.Base_Exploration_Strategy import \
    Base_Exploration_Strategy
...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def __init__(self, config):
        ...
        if "random_episodes_to_run" in self.config.hyperparameters.keys():
            ...
            print("Running {} random episodes".format(
                self.random_episodes_to_run))
        else:
    ...
    def get_updated_epsilon_exploration(self, action_info, epsilon=1.0):
        ...
        if self.exploration_cycle_episodes_length is None:
            epsilon = epsilon / \
                (1.0 + (episode_number / epsilon_decay_denominator))
        else:
            epsilon = self.calculate_epsilon_with_cyclical_strategy(
                episode_number)
        return epsilon
    ...
    def calculate_epsilon_with_cyclical_strategy(self, episode_number):
        ...
        max_epsilon = 0.5
        min_epsilon = 0.001
        increment = (max_epsilon - min_epsilon) / \
            float(self.exploration_cycle_episodes_length / 2)
