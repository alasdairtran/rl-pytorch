...
class Prioritised_Replay_Buffer(Max_Heap, Deque):
    ...
    def __init__(self, hyperparameters, seed=0):
        Max_Heap.__init__(
            self, hyperparameters["buffer_size"], dimension_of_value_attribute=5, default_key_to_use=0)
        Deque.__init__(
    ...
    def pick_experiences_based_on_proportional_td_error(self):
        ...
        deque_sample_indexes = np.random.choice(range(
            len(self.deques_td_errors)), size=self.batch_size, replace=False, p=probabilities)
    ...
    def separate_out_data_types(self, experiences):
        ...
        states = torch.from_numpy(np.vstack(
            [e.value[self.indexes_in_node_value_tuple["state"]] for e in experiences])).float().to(self.device)
        actions = torch.from_numpy(np.vstack(
            [e.value[self.indexes_in_node_value_tuple["action"]] for e in experiences])).float().to(self.device)
        rewards = torch.from_numpy(np.vstack(
            [e.value[self.indexes_in_node_value_tuple["reward"]] for e in experiences])).float().to(self.device)
        ...
        dones = torch.from_numpy(np.vstack(
            [int(e.value[self.indexes_in_node_value_tuple["done"]]) for e in experiences])).float().to(self.device)
    ...
    def calculate_importance_sampling_weights(self, experiences):
        ...
        importance_sampling_weights = [((1.0 / self.number_experiences_in_deque) * (
            self.give_adapted_sum_of_td_errors() / td_error)) ** self.beta for td_error in td_errors]
    ...
    def update_td_errors(self, td_errors):
        ...
        for raw_td_error, deque_index in zip(td_errors, self.deque_sample_indexes_to_update_td_error_for):
            ...
            self.update_overall_sum(
                td_error, self.heap[corresponding_heap_index].key)
