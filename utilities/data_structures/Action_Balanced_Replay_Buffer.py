...
class Action_Balanced_Replay_Buffer(Replay_Buffer):
    ...
    def __init__(self, buffer_size, batch_size, seed, num_actions):
        ...
        self.memories = {action: deque(
            maxlen=self.buffer_size_per_memory) for action in range(self.num_actions)}
    ...
    def pick_experiences(self, num_experiences=None):
        ...
        samples_split_by_action = self.sample_each_action_equally(
            batch_per_action)
        combined_sample = []
    ...
    def calculate_batch_sizes_per_action(self, batch_size):
        ...
        batch_per_action = {
            k: min_batch_per_action for k in range(self.num_actions)}
        current_batch_size = np.sum([batch_per_action[k]
    ...
    def sample_each_action_equally(self, batch_per_action):
        ...
        for action in range(self.num_actions):
            ...
            if action_memory_size >= batch_size_for_action:
                ...
                print("Memory size {} vs. required batch size {}".format(
                    action_memory_size, batch_size_for_action))
                samples_for_action = []
                while len(samples_for_action) < batch_per_action[action]:
                    remainder = batch_per_action[action] - \
                        len(samples_for_action)
                    sampled_experiences = random.sample(
                        memory, min(remainder, action_memory_size))
    ...
    def sample_experiences_with_certain_actions(self, allowed_actions, num_all_actions, required_batch_size):
        ...
        experiences_to_sample = int(
            required_batch_size * float(num_all_actions) / float(num_new_actions))
        experiences = self.sample(num_experiences=experiences_to_sample)
        states, actions, rewards, next_states, dones = experiences
        matching_indexes = np.argwhere(
            (np.in1d(actions.numpy(), allowed_actions)))
        ...
        assert abs(states.shape[0] - required_batch_size) <= 0.05 * \
            required_batch_size, "{} vs. {}".format(
