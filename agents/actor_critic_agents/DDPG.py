...
class DDPG(Base_Agent):
    ...
    def pick_action(self, state=None):
        ...
        if state is None:
            state = torch.from_numpy(
                self.state).float().unsqueeze(0).to(self.device)
    ...
    def critic_learn(self, states, actions, rewards, next_states, dones):
        ...
        self.take_optimisation_step(self.critic_optimizer, self.critic_local,
                                    loss, self.hyperparameters["Critic"]["gradient_clipping_norm"])
    ...
    def calculate_actor_loss(self, states):
        ...
        actor_loss = - \
            self.critic_local(torch.cat((states, actions_pred), 1)).mean()
        return actor_loss
