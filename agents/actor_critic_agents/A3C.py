...
from utilities.Utility_Functions import SharedAdam, create_actor_distribution
...
class A3C(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.actor_critic = self.create_NN(
            input_dim=self.state_size, output_dim=[self.action_size, 1])
...
class Actor_Critic_Worker(torch.multiprocessing.Process):
    ...
    def __init__(self, worker_num, environment, shared_model, counter, optimizer_lock, shared_optimizer,
        ...
        self.local_optimizer = Adam(
            self.local_model.parameters(), lr=0.0, eps=1e-4)
    ...
    def calculate_new_exploration(self):
        ...
        current epsilon"""
        with self.counter.get_lock():
            epsilon = 1.0 / (1.0 + (self.counter.value /
                                    self.epsilon_decay_denominator))
        epsilon = max(0.0, random.uniform(
            epsilon / self.exploration_worker_difference, epsilon * self.exploration_worker_difference))
        return epsilon
    ...
    def calculate_discounted_returns(self):
        ...
        for ix in range(len(self.episode_states)):
            return_value = self.episode_rewards[-(ix + 1)] + \
                self.discount_rate*discounted_returns[-1]
    ...
    def put_gradients_in_queue(self, total_loss):
        ...
        torch.nn.utils.clip_grad_norm_(
            self.local_model.parameters(), self.gradient_clipping_norm)
