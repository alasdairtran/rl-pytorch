...
class TD3(DDPG):
    ...
    def compute_critic_values_for_next_states(self, next_states):
        ...
        with torch.no_grad():
            ...
            critic_targets_next_1 = self.critic_target(
                torch.cat((next_states, actions_next_with_noise), 1))
            critic_targets_next_2 = self.critic_target_2(
                torch.cat((next_states, actions_next_with_noise), 1))
            critic_targets_next = torch.min(torch.cat(
                (critic_targets_next_1, critic_targets_next_2), 1), dim=1)[0].unsqueeze(-1)
    ...
    def critic_learn(self, states, actions, rewards, next_states, dones):
        ...
        self.take_optimisation_step(self.critic_optimizer, self.critic_local,
                                    critic_loss_1, self.hyperparameters["Critic"]["gradient_clipping_norm"])
