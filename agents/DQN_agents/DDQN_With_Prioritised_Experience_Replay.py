...
class DDQN_With_Prioritised_Experience_Replay(DDQN):
    ...
    def __init__(self, config):
        ...
        self.memory = Prioritised_Replay_Buffer(
            self.hyperparameters, config.seed)
    ...
    def learn(self):
        ...
        self.soft_update_of_target_network(
            self.q_network_local, self.q_network_target, self.hyperparameters["tau"])
    ...
    def save_experience(self):
        ...
        self.memory.add_experience(max_td_error_in_experiences, self.state,
                                   self.action, self.reward, self.next_state, self.done)
