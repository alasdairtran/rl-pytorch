...
class Dueling_DDQN(DDQN):
    ...
    def __init__(self, config):
        ...
        Base_Agent.copy_model_over(
            from_model=self.q_network_local, to_model=self.q_network_target)
    ...
    def compute_q_values_for_next_states(self, next_states):
        ...
        max_action_indexes = self.q_network_local(
            next_states)[:, :-1].detach().argmax(1)
    ...
    def calculate_duelling_q_values(self, duelling_q_network_output):
        ...
        q_values = state_value.unsqueeze(
            1) + (duelling_q_network_output[:, :-1] - avg_advantage.unsqueeze(1))
        return q_values
