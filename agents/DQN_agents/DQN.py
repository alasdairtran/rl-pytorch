...
class DQN(Base_Agent):
    ...
    def reset_game(self):
        ...
        self.update_learning_rate(
            self.hyperparameters["learning_rate"], self.q_network_optimizer)
    ...
    def pick_action(self, state=None):
        ...
        self.logger.info(
            "Q values {} -- Action chosen {}".format(action_values, action))
        return action
    ...
    def compute_q_values_for_next_states(self, next_states):
        ...
        Q_targets_next = self.q_network_local(
    ...
    def compute_q_values_for_current_states(self, rewards, Q_targets_next, dones):
        ...
        Q_targets_current = rewards + \
            (self.hyperparameters["discount_rate"]
             * Q_targets_next * (1 - dones))
        return Q_targets_current

    def compute_expected_q_values(self, states, actions):
        """Computes the expected q_values we will use to create the loss to train the Q network"""
        Q_expected = self.q_network_local(states).gather(
            1, actions.long())  # must convert actions to long so can be used as index
        return Q_expected

    def locally_save_policy(self):
        """Saves the policy"""
        torch.save(self.q_network_local.state_dict(),
                   "Models/{}_local_network.pt".format(self.agent_name))
