...
class DIAYN(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.discriminator = self.create_NN(
            self.state_size, self.num_skills, key_to_use="DISCRIMINATOR")
        ...
        self.agent_config.environment = DIAYN_Skill_Wrapper(
            copy.deepcopy(self.environment), self.num_skills, self)
        ...
        # We have to use SAC because it involves maximising the policy's entropy over actions which is also a part of DIAYN
        self.agent = SAC(self.agent_config)

        self.timesteps_to_give_up_control_for = self.hyperparameters[
            "MANAGER"]["timesteps_to_give_up_control_for"]
        ...
        self.manager_agent_config.hyperparameters = self.manager_agent_config.hyperparameters[
            "MANAGER"]
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        self.agent.run_n_episodes(
            num_episodes=self.unsupervised_episodes, show_whether_achieved_goal=False)
        game_full_episode_scores, rolling_results, _ = self.manager_agent.run_n_episodes(
            num_episodes=self.supervised_episodes)
        time_taken = time.time() - start
        pretraining_results = [
            np.min(self.agent.game_full_episode_scores)]*self.unsupervised_episodes
    ...
    def disciminator_learn(self, skill, discriminator_outputs):
        ...
        loss = nn.CrossEntropyLoss()(discriminator_outputs,
                                     torch.Tensor([skill]).long())
    ...
    def get_predicted_probability_of_skill(self, skill, next_state):
        ...
        probability_of_correct_skill = F.softmax(
            predicted_probabilities_unnormalised)[:, skill]
...
class DIAYN_Skill_Wrapper(Wrapper):
    ...
    def step(self, action):
        ...
        new_reward, discriminator_outputs = self.calculate_new_reward(
            next_state)
    ...
    def calculate_new_reward(self, next_state):
        ...
        probability_correct_skill, disciminator_outputs = self.meta_agent.get_predicted_probability_of_skill(
            self.skill, next_state)
        new_reward = np.log(probability_correct_skill + 1e-8) - \
            np.log(self.prior_probability_of_skill)
    ...
    def step(self, skill_chosen):
        ...
        for _ in range(self.timesteps_to_give_up_control_for):
            combined_state = np.concatenate(
                (np.array(self.state), np.array([skill_chosen])))
            action = self.lower_level_agent.pick_action(
                eval_ep=True, state=combined_state)
