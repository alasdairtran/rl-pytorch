...
class HIRO(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.max_sub_policy_timesteps = config.hyperparameters[
            "LOWER_LEVEL"]["max_lower_level_timesteps"]
...
class HIRO_Higher_Level_DDPG_Agent(DDPG):
    ...
    def sample_experiences(self):
        experiences = self.memory.produce_action_and_action_info(
            separate_out_data_types=False)
        assert len(
            experiences[0].state) == self.hyperparameters["max_lower_level_timesteps"] or experiences[0].done
        assert experiences[0].state[0].shape[0] == self.state_size * 2
        assert len(
            experiences[0].action) == self.hyperparameters["max_lower_level_timesteps"] or experiences[0].done
        ...
        states = torch.from_numpy(
            np.vstack([state for state in states])).float().to(self.device)
        actions = torch.from_numpy(
            np.vstack([action for action in actions])).float().to(self.device)
        rewards = torch.from_numpy(
            np.vstack([reward for reward in rewards])).float().to(self.device)
        next_states = torch.from_numpy(
            np.vstack([next_state for next_state in next_states])).float().to(self.device)
        dones = torch.from_numpy(
            np.vstack([int(done) for done in dones])).float().to(self.device)
    ...
    def transform_goal_to_one_most_likely_to_have_induced_actions(self, experience):
        ...
        goal_candidate_state_change = [
            experience.state[-1][:self.state_size] - experience.state[0][:self.state_size]]
        goal_candidate_actual_goal = [experience.state[0][self.state_size:]]
        goal_candidate_state_change_random_iterations = [np.random.normal(
            goal_candidate_state_change[0]) for _ in range(self.number_goal_candidates - 2)]
        goal_candidates = goal_candidate_state_change + \
            goal_candidate_actual_goal + goal_candidate_state_change_random_iterations
        ...
        for goal_ix, goal in enumerate(goal_candidates):
            ...
            for state_ix in range(timesteps_in_experience):
                ...
                log_probability = self.log_probability_lower_level_picks_action(
                    state_obs, goal, action)
    ...
    def log_probability_lower_level_picks_action(self, state, goal, action):
        ...
        and goal as inputs"""
        state_and_goal = torch.from_numpy(np.concatenate(
            (state, goal))).float().unsqueeze(0).to(self.device)
        action_would_have_taken = self.lower_level_policy(
            state_and_goal).detach()
...
class Higher_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    def step(self, goal):
        ...
        # must reset lower level agent to 0 episodes completed otherwise won't run more episodes
        self.HIRO_agent.lower_level_agent.episode_number = 0
        self.HIRO_agent.lower_level_agent.run_n_episodes(
            num_episodes=1, show_whether_achieved_goal=False, save_and_print_results=False)
...
class Lower_Level_Agent_Environment_Wrapper(Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.meta_agent.lower_level_state = self.turn_internal_state_to_external_state(
            state, goal)
    ...
    def step(self, action):
        ...
        if random.random() < 0.008:
            print("Rolling intrinsic rewards {}".format(
                np.mean(self.track_intrinsic_rewards[-100:])))

        self.meta_agent.step_lower_level_states.append(
            self.meta_agent.lower_level_state)
