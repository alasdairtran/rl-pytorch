...
from exploration_strategies.Epsilon_Greedy_Exploration import \
    Epsilon_Greedy_Exploration
from utilities.Parallel_Experience_Generator import \
...
class PPO(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.policy_new = self.create_NN(
            input_dim=self.state_size, output_dim=self.policy_output_size)
        self.policy_old = self.create_NN(
            input_dim=self.state_size, output_dim=self.policy_output_size)
        self.policy_old.load_state_dict(
            copy.deepcopy(self.policy_new.state_dict()))
        self.policy_new_optimizer = optim.Adam(self.policy_new.parameters(
    ...
    def calculate_policy_output_size(self):
        ...
        if self.action_types == "DISCRETE":
            ...
            # Because we need 1 parameter for mean and 1 for std of distribution
    ...
    def step(self):
        ...
        self.update_learning_rate(
            self.hyperparameters["learning_rate"], self.policy_new_optimizer)
    ...
    def policy_learn(self):
        ...
        for _ in range(self.hyperparameters["learning_iterations_per_round"]):
            ...
            loss = self.calculate_loss(
                [all_ratio_of_policy_probabilities], all_discounted_returns)
        ...
        for episode in range(len(self.many_episode_states)):
            ...
            for ix in range(len(self.many_episode_states[episode])):
                return_value = self.many_episode_rewards[episode][-(
                    ix + 1)] + self.hyperparameters["discount_rate"]*discounted_returns[-1]
    ...
    def calculate_all_ratio_of_policy_probabilities(self):
        ...
        all_actions = [[action] if self.action_types ==
                       "DISCRETE" else action for actions in self.many_episode_actions for action in actions]
        all_states = torch.stack(
            [torch.Tensor(states).float().to(self.device) for states in all_states])

        all_actions = torch.stack(
            [torch.Tensor(actions).float().to(self.device) for actions in all_actions])
    ...
    def calculate_loss(self, all_ratio_of_policy_probabilities, all_discounted_returns):
        ...
        potential_loss_value_1 = all_discounted_returns * \
